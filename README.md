...
The popup (toolbar icon) is the sign-in screen and Workspace launcher. After you sign in it shows your name in the header next to a user icon and Settings, plus the Workspace feature buttons enabled for your assigned site (the same set as the Workspace dashboard). Open **Settings** (cog) to switch Workspace site when you have more than one. **Show advanced settings** holds the Laravel **Server host** dropdown and the WordPress session dump. Click the user icon to log out.

...
- **Popup** (`popup/`) is the sign-in screen. The Content Exchange block shows the CEX user name and login/logout (the identity bounce still runs in the service worker). A header cog opens Settings (`<dialog>`) with the configurable Laravel host and WordPress session debug. Keep the popup unless switching to a side panel (which then needs an explicit open trigger).
...
4. Content Exchange login: this Sail app is `http://localhost` (port 80; README often says `http://localhost:8080`). Open the popup → cog → Server `http://localhost` → Save host (allow access) → close Settings → Log in → sign in to CEX (`admin@immediate.co.uk` / `password123` locally) → Connect. Reopen the popup; **Content Exchange** should show `Signed in as {name}`. The editor chat composer enables after login. Ask a question about the draft for a `FlashLite_3_5` reply: the overlay shows the rolling-dot thinking loader, then the reply bubble, a short notification sound, and the user's initials avatar on your turns. Ask to rewrite the title, body, excerpt, SEO, or Open Graph copy and the open editor should update, Save/Update should show unsaved changes, and Undo in WordPress reverts title/body. The post is not saved or published.
